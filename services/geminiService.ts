    ...
    ingredients: {
      ...
      items: {
        ...
        properties: {
          quantity: {
            type: Type.STRING,
            description: 'The amount of the ingredient, e.g., "1 cup" or "2 tbsp".'
          },
          name: {
            type: Type.STRING,
...
const mealPlanSchema = {
  ...
  properties: {
    ...
    summary: { type: Type.STRING, description: "A brief summary of the meal plan's focus (e.g., high-protein, vegetarian)." },
    dailyPlans: {
      type: Type.ARRAY,
      items: {
        type: Type.OBJECT,
...
const shoppingListSchema = {
  type: Type.OBJECT,
  properties: {
    title: { type: Type.STRING, description: "A title for the shopping list, e.g., 'Weekly Groceries'." },
    categories: {
      type: Type.ARRAY,
      ...
      items: {
        type: Type.OBJECT,
        properties: {
          categoryName: { type: Type.STRING, description: "Name of the category, e.g., 'Produce', 'Dairy', 'Meat'." },
          items: {
            type: Type.ARRAY,
...
export const identifyIngredientsFromImage = async (base64ImageData: string, mimeType: string): Promise<string> => {
    const imagePart = {
...
export const getRecipeFromImage = async (base64ImageData: string, mimeType: string): Promise<Recipe> => {
    const imagePart = {
        inlineData: { data: base64ImageData, mimeType },
    };
    const textPart = {
