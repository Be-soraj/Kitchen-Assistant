...
export const CookingAssistant: React.FC<CookingAssistantProps> = ({
  ...
  useEffect(() => {
    ...
    setHistory([
      {
        role: "model",
        text: `Hello! I'm Chef Gemini. How can I help you with the "${recipe.recipeName}" recipe?`,
      },
    ]);
  }, [recipe]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setHistory((prev) => [
        ...
        {
          role: "model",
          text: "Sorry, I had a little trouble in the kitchen. Please try again.",
        },
      ]);
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="fixed bottom-4 right-4 w-full max-w-md bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col animate-[fadeInUp_0.3s_ease-out] z-50">
      ...
      <div
        ...
        className="flex-1 p-4 overflow-y-auto h-80 space-y-4"
      >
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t border-gray-200">
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="bg-orange-500 text-white p-3 rounded-lg hover:bg-orange-600 disabled:bg-gray-400"
          >
            {isLoading ? (
              <Loader />
            ) : (
