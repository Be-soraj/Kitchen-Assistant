...
export const RecipeFinder: React.FC = () => {
    ...
    const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            setError('We couldn\'t identify this dish or generate a recipe. Please try a different image.');
            console.error(err);
        } finally {
            setIsLoading(false);
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100 text-center">
                ...
                <h2 className="mt-4 text-2xl md:text-3xl font-bold text-gray-700">Find Recipe from Image</h2>
                <p className="mt-2 text-gray-500">Have a photo of a delicious meal? Upload it and we'll find the recipe for you!</p>
                <button
                    type="button"
                ...
                >
                    {isLoading ? <><Loader/> Searching...</> : 'Upload a Dish Photo'}
                </button>
            </div>
            ...
            <div className="mt-10">
                {uploadedImage && !isLoading && !generatedRecipe && !error && (
                     <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                        <img src={uploadedImage} alt="Uploaded dish" className="w-full h-auto max-h-96 object-cover"/>
                     </div>
                )}
