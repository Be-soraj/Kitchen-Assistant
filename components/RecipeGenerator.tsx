...
export const RecipeGenerator: React.FC = () => {
  ...
  const handleAnalyzeImage = async (
    ...
    try {
      ...
      const identifiedIngredients = await identifyIngredientsFromImage(
        data,
        mimeType
      );
      setIngredients((prev) =>
        prev ? `${prev}, ${identifiedIngredients}` : identifiedIngredients
      );
    } catch (err) {
  ...
  const handleSubmit = useCallback(
    async (event: React.FormEvent) => {
      ...
      try {
        ...
        setError(
          "An error occurred. Please check your ingredients and try again. The AI chef might be busy."
        );
        console.error(err);
      } finally {
        setIsLoading(false);
    ...
    <div className="max-w-4xl mx-auto">
      <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
        ...
        <p className="text-gray-500 mb-6">
          Enter your ingredients manually or scan them with your camera.
        </p>
