...
// Utility to extract base64 data and mime type from a data URL
...
export const DishVisualizer: React.FC = () => {
    ...
    const handleSubmit = useCallback(async (event: React.FormEvent) => {
        ...
        try {
            ...
            setError('Sorry, we couldn\'t visualize that dish. Please try a different description.');
            console.error(err);
        } finally {
            setIsLoading(false);
        }
    ...
    const handleEdit = async (event: React.FormEvent) => {
        ...
        setError(null);
        try {
            const { data, mimeType } = parseDataUrl(generatedImage);
            const newImageUrl = await editImage(data, mimeType, editPrompt);
            setGeneratedImage(newImageUrl);
            setEditPrompt('');
        } catch (err) {
            setError('Sorry, we couldn\'t edit the image. Please try a different prompt.');
            console.error(err);
        } finally {
            setIsEditing(false);
        }
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <div className="bg-white p-8 rounded-2xl shadow-lg border border-gray-100">
                <h2 className="text-2xl md:text-3xl font-bold text-gray-700 mb-2">Visualize Your Dish</h2>
                <p className="text-gray-500 mb-6">Describe a dish, and our AI will create a photorealistic image of it for you.</p>

                <form onSubmit={handleSubmit}>
                    <div className="flex flex-col gap-4">
                        <div>
                            <label htmlFor="prompt" className="block text-sm font-semibold text-gray-600 mb-2">Dish Description</label>
                            <input
                                id="prompt"
                                value={prompt}
                        ...
                        >
                            {isLoading ? <><Loader /> Visualizing...</> : 'Visualize Dish'}
                        </button>
                    </div>
                </form>
            </div>
